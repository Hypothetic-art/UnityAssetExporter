...
namespace Packages.art.hypothetic.unity.Editor.Scripts.ExporterTreeView
{
	...
	public class TreeModel<T> where T : ExportTreeElement
	{
		IList<T> m_Data;
		T m_Root;
		int m_MaxID;
		...
		IList<int> GetParentsBelowStackBased(ExportTreeElement searchFromThis)
		{
			Stack<ExportTreeElement> stack = new Stack<ExportTreeElement>();
			stack.Push(searchFromThis);

			var parentsBelow = new List<int>();
			while (stack.Count > 0)
			{
				ExportTreeElement current = stack.Pop();
		...
		public void AddElements (IList<T> elements, ExportTreeElement parent, int insertPosition)
		{
			if (elements == null)
			...
			if (parent.children == null)
				parent.children = new List<ExportTreeElement>();

			parent.children.InsertRange(insertPosition, elements.Cast<ExportTreeElement> ());
			foreach (var element in elements)
			{
		...
		public void AddElement (T element, ExportTreeElement parent, int insertPosition)
		{
			if (element == null)
		...
		public void MoveElements(ExportTreeElement parentElement, int insertionIndex, List<ExportTreeElement> elements)
		{
			if (insertionIndex < 0)
			...
			if (parentElement.children == null)
				parentElement.children = new List<ExportTreeElement>();
