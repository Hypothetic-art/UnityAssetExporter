...
namespace Packages.art.hypothetic.unity.Editor.Scripts.ExporterTreeView
{
	...
	{
		public static void TreeToList<T>(T root, IList<T> result) where T : ExportTreeElement
		{
			if (result == null)
		...
		public static T ListToTree<T>(IList<T> list) where T : ExportTreeElement
		{
			// Validate input
			...
			for (int parentIndex = 0; parentIndex < list.Count; parentIndex++)
			{
				...
				// Fill child array
				List<ExportTreeElement> childList = null;
				if (childCount != 0)
				{
					childList = new List<ExportTreeElement>(childCount); // Allocate once
					childCount = 0;
					for (int i = parentIndex + 1; i < list.Count; i++)
					{
						if (list[i].depth == parentDepth + 1)
		...
		public static void ValidateDepthValues<T>(IList<T> list) where T : ExportTreeElement
		{
			if (list.Count == 0)
		...
		public static void UpdateDepthValues<T>(T root) where T : ExportTreeElement
		{
			if (root == null)
		...
		static bool IsChildOf<T>(T child, IList<T> elements) where T : ExportTreeElement
		{
			while (child != null)
			{
		...
		public static IList<T> FindCommonAncestorsWithinList<T>(IList<T> elements) where T : ExportTreeElement
		{
			if (elements.Count == 1)
